    ...
    return residents
      .filter(
        (r) =>
          ...
          (r.document && r.document.toLowerCase().includes(searchQuery.toLowerCase()))
            ...
            {filteredResidents.length > 0 ? (
              filteredResidents.map((resident) => (
                <TableRow key={resident.id}>
                  <TableCell className="font-medium max-w-[200px] truncate">{resident.name}</TableCell>
                  <TableCell>{resident.apartment} / {resident.block}</TableCell>
                  <TableCell className="hidden md:table-cell">{resident.document || '-'}</TableCell>
                  <TableCell className="hidden sm:table-cell">{resident.phone || '-'}</TableCell>
